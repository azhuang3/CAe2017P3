...
public class CAe2017P3 extends PApplet {
...
float t=0.5f;               // current animaiton time
...
public void draw()            // loops forever
  {
  ...
  if(showStrobeFrames) 
    ...
    for(int j=0; j<n; j++)
      {
      fill(255-(200.f*j)/n,(200.f*j)/n,155); 
  ...
  if(animating) {t+=0.01f; if(t>=1) {t=1; animating=false;}} // increments timing and stops when animation is complete
...
public void mouseMoved() // press and hold the key you want and then move the mouse (do not press any mouse button)
  {
  if(keyPressed)
    {
    ...
    if(key=='q') {
      left_theta += (mouseX-pmouseX) * 0.1f;
    }
    }
  change=true; // reset to render movie frames for which something changes
  }
...
public void paintShape(float x, float y, float r, float b, float d)
  {
...
public void showConstruction(float x, float y, float r, float b, float d) 
  {
  // compute blend radii
  float p=_p, q=_q; // use gobal values (user controlled) in case we do not want to recompute them automatically
  // if(computingBlendRadii)
...
public float wrapTo2PI(float angle) {
  if (angle < 0) {
    while (angle < 0) {
      angle += 2 * PI;
    }
...
public void computeParametersForAnimationTime(float t) { // computes parameters x, y, r, b, d for current t value
    // Linear default
    // x = x0 + t * (x1-x0);
    ...
    left_theta = lerp(4.1f * PI / 4, 5.9f * PI / 4, t);
    println("Drawing: " + left_theta);
    PVector leftCircle = calcCircle(x, y, lerp(4.1f * PI / 4, 5.9f * PI / 4, t), r);
    b = x - leftCircle.x;
    _p = leftCircle.y;

    PVector rightCircle = calcCircle(x, y, lerp(4.1f * PI / 4 + PI / 2, 5.9f * PI / 4 + PI / 2, t), r);
    d = rightCircle.x - x;
    _q = rightCircle.y;
...
public PVector calcCircle(float x, float y, float theta, float r) {
    // tangent point
    ...
    float center_theta = tangent_theta / 2.0f;
...
public void flipCirc(float x, float y) {
  // ellipse(x, g - y, 5, 5);
}

public void flipLine(float x, float y, float x1, float y1) {
    // line(x, g - y, x1, g - y1);
}

//*********** TO BE PROVIDED BY STUDENTS  
// compute blend radius tangent to x-axis at point (0,0) and circle of center (b,y) and radius r   
public float blendRadius(float b, float y, float r) {
    return 0; // replace with your formula
}
  public void settings() {  size(1000, 400, P2D); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CAe2017P3" };
    if (passedArgs != null) {
